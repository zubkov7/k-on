...
std::string Client::json_to_songs(boost::property_tree::ptree &response,std::string url) {
    ...
    for (auto song : songs_array) {
        ...
        str += "<div> <a href='/similarsong?song_id=" + std::to_string(song.second.get<int>("id")) + "'>" +
               ...
               +"<a href='/like?song_id=" + std::to_string(song.second.get<int>("id")) + "&like=0&next=" +url + "' > Dislike </a>" +
