...
DbRecommendationSystem::DbRecommendationSystem(const std::string &database) : worker(
        DbWorkerRecommendations(database)) {}

DbRecommendationSystem::~DbRecommendationSystem() = default;

std::vector<Song> DbRecommendationSystem::get_recommendations(int user_id, int count) {
...
std::vector<Song> DbRecommendationSystem::get_popular(int count) {
    return worker.get_popular_songs(count);
}

std::vector<Song> DbRecommendationSystem::get_new(int count) {
